...
if [ -f /mnt/mmcblk0p2/R4S*.gz ]; then
	...
	wget https://github.com/DHDAXCW/NanoPi-R4S-2021/releases/download/Lean-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/Lean-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)-openwrt-rockchip-armv8-friendlyarm_nanopi-r4s-squashfs-sysupgrade.img.gz
	if [ -f /mnt/mmcblk0p2/R4Z*.gz ]; then
